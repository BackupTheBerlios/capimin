...
def ShowSend(user,changepage="change.cgi",newpage="",dldpage="",removepage=""):    
    ...
    print '   <th>%s</th><th>%s</th><th>%s</th><th>%s</th>' %  (webmin.text['index_id'],webmin.text['index_dialstring'],
                                                            webmin.text['index_addressee'],webmin.text['index_tries'])
    print '   <th>%s</th><th>%s</th><th><b>&nbsp;</b></th><th>%s</th>\n </tr>' % (webmin.text['index_nexttry'],
                                                            webmin.text['index_subject'],webmin.text['index_toabort'])
    ...
    for job in files:
        ...
        print '   <td><input TYPE="TEXT" NAME="formDialAddressee" SIZE="15" MAXLENGTH="40" value="%s"></td>' %\
        ...
        print '        &nbsp;&nbsp;<input name="hour" size="2" maxlength="2" value="%s">:<input name="min" size="2"'\
              ' maxlength="2" value="%02d"></td>' % (starttime[3],starttime[4])
        ...
        print '   <td><input TYPE="TEXT" NAME="formSubject" SIZE="30" MAXLENGTH="40" value="%s"></td>' %\
              cs_helpers.getOption(control,"GLOBAL","subject","")
        ...
        print '   <input type="hidden" name="cindex" value="0">' # dummy
        print '   <td><input type="submit" name="schange" value="%s"></td>\n  </form>' % webmin.text['index_change']
        urlparams = urllib.urlencode({'cjobid': jobid, 'cslist': 'faxsend'})
        print '   <td>&nbsp;&nbsp;<i><a href="%s?%s">%s</a></i></td>' % (removepage,urlparams,
                                                                         webmin.text['index_toabort'])
        print ' </tr>'
